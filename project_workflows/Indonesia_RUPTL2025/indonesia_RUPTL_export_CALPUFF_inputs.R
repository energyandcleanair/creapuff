...
emis_long %<>% rename(Velocity..m.s.=contains('Velocity'),
                      Diameter..m.=contains('Diameter'),
                      Exit.Temp....=contains('Temp'),
                      Height..m.=contains('Height'))
...
emis_base %>% saveRDS(file.path(emissions_dir, 'indonesia_RUPTL_base_emissions, with stack data.RDS'))

#cluster emissions
if("cluster" %notin% names(emis_base)) {
...
emis_base %>% 
  group_by(CFPP.name, emission_names, cluster, is_new_cluster) %>% 
  select(Longitude, Latitude, all_of(stack_cols), MW, COD, SO2_control, pollutant, emissions_t) %>% 
