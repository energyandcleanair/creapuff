...
case_description='captive coal power plants in North Maluku'
case_name='malut-captive-plants'
...
output_dir <- file.path(project_dir, 'animation/Malut'); dir.create(output_dir)
...
emis %>% 
  filter(CFPP.name %in% malut_plants$Plant) %>% 
...
plot_bb = point_sources_to_plot %>% st_transform(crs=crs(grids$gridR)) %>% extent() %>% add(700)
...
plot_bb = point_sources_to_plot %>% st_transform(crs=4326) %>% extent() %>% add(c(-2.7,2.2,-2.4,2.25))
