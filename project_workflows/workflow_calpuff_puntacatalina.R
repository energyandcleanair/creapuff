...
if (emission_type == "constant"){
  ...
  input_xls <- file.path(emissions_dir,"CTPC_emissions_final.xlsx") # File where constant-emission data are specified
...
if (emission_type == "constant") {
  ...
  # Exclude sources outside domain. # If NULL, no plant in the selected area (maybe lat and long have been switched)  
...
nesfact_range = c(150,75,35,10) # c(125,50,25,5)  # c(150,75,25,5)  # c(125,75,25,5)  
...
# --- Two main scenarios : 
if (i_Sc==1) {scenario_prefix <- "ScA"; included_stations <- emissions_data$emission_names[emissions_data$Scenario=='Declared emissions']}
if (i_Sc==2) {scenario_prefix <- "ScB"; included_stations <- emissions_data$emission_names[emissions_data$Scenario=='Real emissions – best case']}
if (i_Sc==3) {scenario_prefix <- "ScC"; included_stations <- emissions_data$emission_names[emissions_data$Scenario=='Real emissions – worst case']}
if (i_Sc==4) {scenario_prefix <- "ScD"; included_stations <- emissions_data$emission_names[emissions_data$Scenario=='Real emissions – average']}
