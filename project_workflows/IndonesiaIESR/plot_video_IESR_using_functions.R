...
tseries_rds_to_raster(infiles, grids, times=times)

#sum concentrations for multiple simulations for each timestep
sum_tseries_rasters(infiles, case_name='allsources', times=times, overwrite=F)
...
calpuff_files_all <- get_calpuff_files(filetype='tseries', dir=output_dir) %>% 
  ...
  mutate(title=paste(speciesName, 'concentration from coal power plants in Indonesia'),
