...
list.files('R', '\\.R$', full.names=T) %>% lapply(source)
...
emissions_dir <- "project_workflows/Indonesia_RUPTL2025/data" #file.path(project_dir,"emissions") # Directory where arbitrary-varying emission files are stored
...
#
runs = unique(emissions_data$emission_names[emissions_data$is_new_cluster])
...
if(file.exists(rec_file)) { receptors <- readRDS(rec_file)
} else { receptors = list() }
...
for(run in queue) {
  ...
  emissions_data_run$AQCS <- ifelse(emissions_data_run$FGD, "ESP+wFGD", "ESP")
...
#queue=runs
file.path(output_dir, paste0(queue, '_CALPUFF_7.0.inp')) %>% split(1) -> batches
...
calpuff_done=plants[paste0(plants, '.CON') %>% file.path(output_dir, .) %>% file.exists()]
