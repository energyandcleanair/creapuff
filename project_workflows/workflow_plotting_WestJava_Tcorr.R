...
calpuff_files %>% 
  filter(scenario %in% scenarios_to_process, species %in% c('pm25', 'no2', 'so2', 'tpm10'), period=='annual') %>% 
  as.data.frame() %>%
...
calpuff_files_all <- get_calpuff_files(ext=".tif", gasunit = 'ug', dir=input_dir, hg_scaling=1e-3) %>% 
  ...
  mutate(scenario_description='Banten Suralaya complex') %>% 
  mutate(title=make_titletxt(., line_break = F),
         subtitle=case_when(grepl('bmas', scenario)~'current (mean)',
                            grepl('bhas', scenario)~'current (maximum)',
                            grepl('batas', scenario)~'best available technology',
                            grepl('coas', scenario)~'compliance with Indonesian regulation'),
...
#                contour_type='both',
#                color_scale_basis_scenario='bhas',
